...
public class ArtemisTransmutingComponentBenchmark extends DominionBenchmark {
    ...
    EntityTransmuter entityTransmuter;
    ...
    public static void main(String[] args) throws IOException {
        org.openjdk.jmh.Main.main(
                new String[]{ArtemisTransmutingComponentBenchmark.class.getName()}
        );
    }
    ...
    public void setup() {
        ...
        entityTransmuter = new EntityTransmuterFactory(world).remove(C1.class).add(C0.class).build();
    ...
    @Benchmark
    public void transmuteComponentWith01(Blackhole bh) {
        for (int i = 0; i < size; i++) {
            bh.consume(transmute(entities1[i]));
        }
        world.process();
    }

    @Benchmark
    public void transmuteComponentWith02(Blackhole bh) {
        for (int i = 0; i < size; i++) {
            bh.consume(transmute(entities2[i]));
        }
        world.process();
    }

    @Benchmark
    public void transmuteComponentWith04(Blackhole bh) {
        for (int i = 0; i < size; i++) {
            bh.consume(transmute(entities4[i]));
        }
        world.process();
    }

    @Benchmark
    public void transmuteComponentWith08(Blackhole bh) {
        for (int i = 0; i < size; i++) {
            bh.consume(transmute(entities8[i]));
        }
        world.process();
    }

    private int transmute(int entity) {
        entityTransmuter.transmute(entity);
        return entity;
    }
